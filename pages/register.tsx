...
import { FaHandHoldingHeart, FaArrowLeft } from "react-icons/fa";
...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await fetch("https://hmif-peduli-backend.vercel.app/users/register", {
        method: "POST",
  ...
  return (
    <div className="h-[100vh] w-full flex flex-col justify-center items-center">
      ...
      <div className="border-[#cfcfcf] shadow-md border-1 h-[570px] w-[335px] rounded-[20px] bg-[#fff] px-7 py-14 flex flex-col">
        <div className="items-center flex gap-2 justify-center h-[5vh]">
          <FaHandHoldingHeart className="text-[#4E1F00] w-10 h-10" />
          <span className="tracking-wide text-[20px] font-semibold">HMIFPeduli</span>
        </div>
        <h1 className="text-center m-[1.3rem]">Buat akun baru dan mulai berdonasi.</h1>
        <form onSubmit={handleSubmit} className="flex flex-col gap-2">
          ...
          <button
            type="submit"
            className="border-1 text-[13px] text-[#FFFFFF] bg-[#74512D] hover:bg-[#5f4b37] focus:outline-2 focus:outline-offset-1 focus:outline-[#74512D] border-[#a7a7a7] shadow-md rounded-[7px] h-9.5 mt-2"
          >
            Daftar
          </button>
        </form>
        {message && <p className="text-center text-red-500 text-[12px] mt-2">{message}</p>}
        <h1 className="pt-3 pb-2 text-center text-[12px] mt-1">Sudah punya akun?</h1>
        <a
          href="/login"
          className="text-center flex justify-center items-center mt-1 hover:underline text-sky-600"
        >
          <h1 className="text-[12px] mt-1">Masuk di sini</h1>
        </a>
      </div>
    </div>
  );
